...
================================================
{다음 과정을 하나의 파이프라인으로 수행}
1. Intel RealSense D435 카메라로 작업 영역을 촬영
2. YOLO Segmentation 모델로 각 큐브의 위치와 회전각을 검출
3. ResNet 기반 이상 탐지 모델로 각 큐브를 GOOD 또는 BAD로 판정
4. 픽셀 좌표와 깊이값을 카메라 3차원 좌표로 변환
5. 외부 파라미터 행렬을 이용해 카메라 좌표를 로봇 기준 좌표로 변환
6. 두산로보틱스 M0609 협동로봇이 큐브를 파지
7. 이상 탐지 결과에 따라 GOOD 영역 또는 BAD 영역으로 큐브를 옮김
8. 파지 과정에서는 순응제어와 힘 조건 검사를 이용해 충돌을 감지

{주의사항}
--------
- YOLO 모델 경로, 이상 탐지 모델 경로, Place 위치는 실행 환경에 맞게 수정해야 합니다.
- Camera-to-Robot 외부 파라미터는 해당 카메라와 로봇 설치 상태에서 측정된 값이어야 합니다.
- 실제 로봇 실행 전에는 낮은 속도에서 작업 영역과 좌표값을 충분히 검증해야 합니다.
"""
import rclpy
import DR_init
import sys
